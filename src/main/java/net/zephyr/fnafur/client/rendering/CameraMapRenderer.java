...
public class CameraMapRenderer {
    public void render(PoseStack matrices, MultiBufferSource.BufferSource vertexConsumers, double cameraX, double cameraY, double cameraZ){
        ...
        CompoundTag MainData = ItemNbtUtil.getNbt(player.getMainHandStack());
        CompoundTag OffData = ItemNbtUtil.getNbt(player.getOffHandStack());
        ...
        if((player.getMainHandStack().isOf(ItemInit.TAPEMEASURE) || (player.getMainHandStack().isOf(ItemInit.PAINTBRUSH)) && player.getOffHandStack().isOf(ItemInit.TABLET))){
            ...
            if(!OffData.getList("CamMap").get().isEmpty()){
                ListTag mapNbt = OffData.getList("CamMap").get().copy();

                HitResult blockHit = Minecraft.getInstance().player.raycast(20.0, 0.0f, false);
                ...
                for(int i = 0; i < mapNbt.size(); i++){
                    if(mapNbt.getLongArray(i).get().length > 0){
                        ...
                        if(Minecraft.getInstance().player.getMainHandStack().isOf(ItemInit.TAPEMEASURE) && (!MainData.getBoolean("hasCorner").get() && line.contains(pos.toCenterPos()))){
                            ...
                            ListTag mapList = line.contains(pos.toCenterPos()) ? list : mapNbt;
...
//    void renderLine(PoseStack matrices, BlockPos pos1, BlockPos pos2, float red, float green, float blue, float alpha, boolean selected, ListTag mapNbt){
...
//        float distance = Mth.abs(Mth.sqrt(((xEnd - xStart) * (xEnd - xStart)) + ((zEnd - zStart) * (zEnd - zStart))));
//
...
//        };
//    }
//    int getLinePartShape(BlockPos pos, BlockPos pos1, BlockPos pos2, Direction direction, Direction lineDirection, ListTag mapNbt){
...
//    }
//
//    boolean isPartOfLine(BlockPos pos, ListTag mapNbt){
//        for(int i = 0; i < mapNbt.size(); i++) {
//            if (mapNbt.getLongArray(i).get().length > 0) {
//                BlockPos pos1 = BlockPos.fromLong(mapNbt.getLongArray(i).get()[0]);
//                BlockPos pos2 = BlockPos.fromLong(mapNbt.getLongArray(i).get()[1]);
