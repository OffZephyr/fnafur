...
public class TriggerBlockScreen extends InWorldScreen {

    static int COLOR1 = 0xFFF62C39;
    static int COLOR2 = 0xFFFF4C1B;
    static int COLOR3 = 0xFF882233;
    static int COLOR4 = 0xAA010000;
    static int COLOR5 = 0xFF5A0213;
    ...
    List<String> ActiveSettingList = new ArrayList<>();
    List<String> SettingList = new ArrayList<>();
    List<String> AnimationList = new ArrayList<>();
    List<String> AmbientSoundList = new ArrayList<>();

    public TriggerBlockScreen(Component title, CompoundTag nbt, long l) {
        super(title, nbt, l);
    ...
    public TriggerBlockScreen(Component text, CompoundTag CompoundTag, Object o) {
        super(text, CompoundTag, o);
        data = CpuData.fromNbt(CompoundTag.getCompound("data").orElse(new CompoundTag()));

        ActiveSettingList = CompoundTag.read("used_settings", Codec.list(Codec.STRING)).orElse(new ArrayList<>());
        selectedIndex++;
    ...
    public Vec3 getCameraPos() {
        ...
        if(blockState.is(BlockInit.TRIGGER_BLOCK)){
            Vec3 offset = blockState.getValue(TriggerBlock.FACING).getUnitVec3().scale(0.15f);
            Vec3 offset2 = blockState.getValue(TriggerBlock.FACING).getClockWise().getUnitVec3().scale(-0.055f);
            return getBlockPos().getCenter().add(0, 0.3f, 0).add(offset.add(offset2));
        }
    ...
    public Vector3f getCameraAngle() {
        BlockState blockState = Minecraft.getInstance().level.getBlockState(getBlockPos());
        if(blockState.is(BlockInit.TRIGGER_BLOCK)){
            return new Vector3f(blockState.getValue(TriggerBlock.FACING).toYRot() + 180, 0, 0);
        }
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        if(ClientHook.tickTransitionToScreen == 1){
            ...
            if(age < 2){
                ...
                drawResizableText(context, font, MenuInitials, 5, width/2f, height/2f - 20, COLOR2, 0x00000000, false, true);
                if(age > 0.45f){
                    drawResizableText(context, font, MenuName, 0.9f, width/2f - 2, height/2f + 20, COLOR1, 0x00000000, false, true);
                }
            ...
            drawResizableText(context, font, MenuInitials, 2.7f, width/4f - 2, height/28f + 2, COLOR2, 0x00000000, false, false);
            drawResizableText(context, font, MenuName, 0.475f, width/4f - 4, height/28f + 23, COLOR1, 0x00000000, false, false);
            ...
            drawResizableText(context, font, Controls3, 0.9f, -12 + width - (width/4f) - controlWidth3, height/28f + 0, COLOR2, 0x00000000, false, false);
            drawResizableText(context, font, Controls, 0.75f, -12 + width - (width/4f) - controlWidth1, height/28f + 11, COLOR1, 0x00000000, false, false);
            drawResizableText(context, font, Controls2, 0.75f, -12 + width - (width/4f) - controlWidth2, height/28f + 20, COLOR1, 0x00000000, false, false);

            context.fill(width/4 - 4, height/28 + 31, -12 + width - (width/4), height/28 + 32, COLOR2);

            int list = age < 2.25f ? 0 : age < 2.321f ? 5 : age < 2.67 ? 8 : 10;
            for(int i = 0; i < Math.min(getList().size(), list); i++){
                ...
                boolean isEnabled = ActiveSettingList.contains(name);

                int normalColor = isEnabled ? COLOR1 : COLOR5;
                int selectedColor = isEnabled ? COLOR2 : COLOR3;
                int color = title ? COLOR2 : i == selected ? selectedColor : normalColor;
                ...
                Component text = Component.literal("").append(nameText);

                if(isEnabled){
                    text = Component.literal("").append(Component.literal("[-] ").toFlatList(style).getFirst()).append(text).append(Component.literal(": ").toFlatList(style).getFirst()).append(selectionText);
                }
                ...
                context.fill(-4 + (int)x, (int) y - 3, -12 + width - (width/4), (int) y + 11, COLOR4);
    ...
    public boolean mouseClicked(MouseButtonEvent click, boolean doubled) {
        ...
        if(subWindow == 1){
            ...
            if(ActiveSettingList.contains(index)){

                int offset = scrollAmount;
                int i = selectedIndex + offset;
                float x = width / 4f;
                float y = 3 + height / 6f + i * 18;
                if (isOnButton(click.x(), click.y(), -4 + (int) x, (int) y - 3, 20, 14)) {
                    ActiveSettingList.remove(index);
                }
                else{
                    if (click.button() == 0) {
    ...
    public void onClose() {
        ...
        nbt.store("used_settings", Codec.list(Codec.STRING), ActiveSettingList);
        //Minecraft.getInstance().player.setStackInHand(Minecraft.getInstance().player.preferredHand, stack);
        //ClientPlayNetworking.send(new UpdateMainHandItemC2SPayload(nbt));

        ClientPlayNetworking.send(new UpdateBlockNbtC2SPayload(getBlockPos().asLong(), nbt));
    }
