...
public class TriggerBlockScreen extends InWorldScreen {
    ...
    public TriggerBlockScreen(Text title, NbtCompound nbt, long l) {
        super(title, nbt, l);
    ...
    public TriggerBlockScreen(Text text, NbtCompound nbtCompound, Object o) {
        super(text, nbtCompound, o);
        data = CpuData.fromNbt(nbtCompound.getCompound("data").orElse(new NbtCompound()));

        ActiveSettingList = nbtCompound.get("used_settings", Codec.list(Codec.STRING)).orElse(new ArrayList<>());
        selectedIndex++;
    ...
    public Vec3d getCameraPos() {
        ...
        if(blockState.isOf(BlockInit.TRIGGER_BLOCK)){
            Vec3d offset = blockState.get(TriggerBlock.FACING).getDoubleVector().multiply(0.15f);
            Vec3d offset2 = blockState.get(TriggerBlock.FACING).rotateYClockwise().getDoubleVector().multiply(-0.055f);
            return getBlockPos().toCenterPos().add(0, 0.3f, 0).add(offset.add(offset2));
        }
    ...
    public Vector3f getCameraAngle() {
        BlockState blockState = MinecraftClient.getInstance().world.getBlockState(getBlockPos());
        if(blockState.isOf(BlockInit.TRIGGER_BLOCK)){
            return new Vector3f(blockState.get(TriggerBlock.FACING).getPositiveHorizontalDegrees() + 180, 0, 0);
        }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if(ClientHook.tickTransitionToScreen == 1){
            ...
            if(age < 2){
                ...
                drawResizableText(context, textRenderer, MenuInitials, 5, width/2f, height/2f - 20, COLOR2, 0x00000000, false, true);
                if(age > 0.45f){
                    drawResizableText(context, textRenderer, MenuName, 0.9f, width/2f - 2, height/2f + 20, COLOR1, 0x00000000, false, true);
                }
            ...
            drawResizableText(context, textRenderer, MenuInitials, 2.7f, width/4f - 2, height/28f + 2, COLOR2, 0x00000000, false, false);
            drawResizableText(context, textRenderer, MenuName, 0.475f, width/4f - 4, height/28f + 23, COLOR1, 0x00000000, false, false);
            ...
            drawResizableText(context, textRenderer, Controls3, 0.9f, -12 + width - (width/4f) - controlWidth3, height/28f + 0, COLOR2, 0x00000000, false, false);
            drawResizableText(context, textRenderer, Controls, 0.75f, -12 + width - (width/4f) - controlWidth1, height/28f + 11, COLOR1, 0x00000000, false, false);
            drawResizableText(context, textRenderer, Controls2, 0.75f, -12 + width - (width/4f) - controlWidth2, height/28f + 20, COLOR1, 0x00000000, false, false);
            ...
            for(int i = 0; i < Math.min(getList().size(), list); i++){
                ...
                Text text = Text.literal("").append(nameText);

                if(isEnabled){
                    text = Text.literal("").append(Text.literal("[-] ").getWithStyle(style).getFirst()).append(text).append(Text.literal(": ").getWithStyle(style).getFirst()).append(selectionText);
                }
    ...
    public void close() {
        ...
        nbt.put("used_settings", Codec.list(Codec.STRING), ActiveSettingList);
