...
public class SoundUtils {
    ...
    public static void playMutableSound(Level world, Entity entity, String sound, float volume, float pitch){
        if(world instanceof ServerLevel world2) {
            for (ServerPlayer entity2 : PlayerLookup.all(world2.getServer())) {
    ...
    public static void playMutableSound(Entity entity, SoundEvent sound, float volume, float pitch) {
        SoundInstance si = new CustomSoundInstance(sound, SoundSource.PLAYERS, volume, pitch, entity, RandomSource.createThreadSafe().nextLong(), SoundsInit.soundRepeats(sound));
        Minecraft.getInstance().getSoundManager().play(si);
        sounds.put(getKey(entity, sound), si);
    }
    @Environment(EnvType.CLIENT)
    public static void stopSound(Level world, Entity entity, String sound){
        if(world instanceof ServerLevel world2) {
            for (ServerPlayer entity2 : PlayerLookup.all(world2.getServer())) {
    ...
    public static String getKey(Entity entity, SoundEvent sound){
        return sound.location().getPath() + entity.getId();
    }
}
