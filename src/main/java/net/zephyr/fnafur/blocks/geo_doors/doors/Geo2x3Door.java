...
public class Geo2x3Door extends GeoDoor {
    public Geo2x3Door(Settings settings) {
        super(settings);
    }
    ...
    public List<BlockPos> doorPos(BlockState state, BlockPos origin){
        ...
        door.add(origin);
        door.add(origin.up());
        door.add(origin.up().up());
        BlockPos offset = origin.offset(direction.rotateYCounterclockwise());
        door.add(offset);
        door.add(offset.up());
        door.add(offset.up().up());
        return door;
    }
    ...
    public Box getEntityArea(BlockState state, BlockPos pos) {
        ...
        if(state.get(GeoDoor.FACING).getAxis() == Direction.Axis.Z){
            box = new Box(pos.getX() - 1f, pos.getY() - 1, pos.getZ() - 2.5f, pos.getX() + 3f, pos.getY() + 2, pos.getZ() + 3.5f);
        }
        else {
            box = new Box(pos.getX() - 2.5f, pos.getY() - 1, pos.getZ() - 1f, pos.getX() + 3.5f, pos.getY() + 2, pos.getZ() + 3f);
        }
        return box;
    }
