...
vec3 sampleLightMul() {
    ...
    return texture(LightBuffer, uv).rgb;
}
...
void main() {
    ...
    vec3 lightMul = sampleLightMul();
    vec3 litRgb = base.rgb * lightMul;
...
//    vec2 uv = gl_FragCoord.xy / vec2(textureSize(LightBuffer, 0));
//    fragColor = vec4(texture(LightBuffer, uv).rgb, 1.0);
